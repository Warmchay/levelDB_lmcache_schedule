...
static int FLAGS_YCSB_uniform_distribution = 1;
...
// Size of each value
static int FLAGS_value_size = 232;
...
static int FLAGS_dcache_size = -1;  // 22-7-11 data cache size

static int FLAGS_wss = 1000; //22-7-11 缓存命中率观察窗口
...
class Stats {
 private:
  ...
  //Histogram hist_;
  Histogram hist_w;  // 22-3分别记录读写尾时延
  Histogram hist_r;
  std::string message_;
 ...
 public:
  ...
  void Merge(const Stats& other) {
    ...
    hist_w.Merge(other.hist_w);  //22-3
    hist_r.Merge(other.hist_r);
    done_ += other.done_;
    bytes_ += other.bytes_;
  ...
  // 22-3 分别为读写操作统计时延分布
  void FinishedSingleOp(bool flag) {  // true-write, false-read
    if (FLAGS_histogram) {
  ...
  void Report(const Slice& name) {
    ...
    if (FLAGS_histogram) {
      ...
      std::fprintf(stdout, "Microseconds per op:\n%s\n",
                   hist_w.ToString().c_str());
      std::fprintf(stdout, "Microseconds per op:\n%s\n",
                   hist_r.ToString().c_str());  // 22-3
    }
    std::fflush(stdout);
  }
};
...
class Benchmark {
 private:
  Cache* cache_;
  Cache* dcache_;  //22-7-11 data cache
 ...
 public:
  Benchmark()
      : cache_(FLAGS_cache_size >= 0 ? NewLRUCache(FLAGS_cache_size) : nullptr),
        dcache_(FLAGS_dcache_size >= 0 ? NewLRUCache(FLAGS_dcache_size) : nullptr),  // 22-7-11 data cache 
  ...
  void Run() {
    ...
    while (benchmarks != nullptr) {
      ...
      } else if (name == Slice("ycsba")){  // 4-8
        method = &Benchmark::YCSBWorkloadA;
      } else if (name == Slice("ycsbb")){  // 4-8
        method = &Benchmark::YCSBWorkloadB;
      } else if (name == Slice("ycsbc")){  // 4-8
        method = &Benchmark::YCSBWorkloadC;
      } else if (name == Slice("ycsbd")){  // 4-8
        method = &Benchmark::YCSBWorkloadD;
      } else if (name == Slice("ycsbe")){  // 4-8
        method = &Benchmark::YCSBWorkloadE;
      } else if (name == Slice("ycsbf")){  // 4-8
      ...
      if (method != nullptr) {
        ...
        // 22-3 结束的时候结一些时间结束当前负载
        //g_env->SleepForMicroseconds(30000000);
      }
    }
  }
 ...
 private:
  ...
  void RunBenchmark(int n, Slice name,
    ...
    for (int i = 0; i < n; i++) {
      ...
      arg[i].thread = new ThreadState(i, /*seed=*/1000 + i);
  ...
  void Crc32c(ThreadState* thread) {
    ...
    while (bytes < 500 * 1048576) {
      ...
      thread->stats.FinishedSingleOp(true); // 22-3
      bytes += size;
    }
  ...
  void Open() {
    ...
    options.data_cache = dcache_;  //22-7-11
    options.ws_size = FLAGS_wss;  //22-7-11
  ...
  void ReadSequential(ThreadState* thread) {
    ...
    for (iter->SeekToFirst(); i < reads_ && iter->Valid(); iter->Next()) {
      ...
      thread->stats.FinishedSingleOp(false);  //22-3
      ++i;
    }
    delete iter;
    thread->stats.AddBytes(bytes);
  ...
  void YCSBWorkloadA(ThreadState* thread) {
    ...
    thread->stats.AddBytes(bytes); 
    char msg[100];
    snprintf(msg, sizeof(msg), "(reads:%d  writes:%d  total:%d  found:%d)",
  ...
  void YCSBWorkloadB(ThreadState* thread) {
    ...
    thread->stats.AddBytes(bytes); 
    char msg[100];
    snprintf(msg, sizeof(msg), "(reads:%d  updates:%d  total:%d  found:%d)",
             reads_done, updates_done, num_, found);
    thread->stats.AddMessage(msg);
  }
  ...
  void YCSBWorkloadD(ThreadState* thread) {
    ReadOptions options;
    RandomGenerator gen;
    init_latestgen(FLAGS_num);  // 依赖zipf
    init_zipf_generator(0, FLAGS_num);
    ...
    for (int i = 0; i < num_; i++) {
      ...
      if (FLAGS_YCSB_uniform_distribution){
        ...
        k = (int)(next_value_latestgen() % FLAGS_num);           
      }
      char key[100];
      snprintf(key, sizeof(key), "%016d", k);
  ...
  void YCSBWorkloadE(ThreadState* thread) {
    ...
    // the number of iterations is the larger of read_ or write_
    for (int i = 0; i < reads_; i++) {  //22-4 控制scan的执行数
      int k;
      if (FLAGS_YCSB_uniform_distribution){
        //Generate number from uniform distribution            
        k = thread->rand.Next() % FLAGS_num;
    ...
    thread->stats.AddBytes(bytes);
    char msg[100];
    snprintf(msg, sizeof(msg), "( scans:%d writes:%d total:%d found:%d)",
             scans_done, writes_done, num_, found);
    thread->stats.AddMessage(msg);   
  }
...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (leveldb::Slice(argv[i]).starts_with("--benchmarks=")) {
      ...
      FLAGS_cache_size = n;
    } else if (sscanf(argv[i], "--dcache_size=%d%c", &n, &junk) == 1) {  // 22-7-11将数据缓存大小初始值设为参数
      FLAGS_dcache_size = n;
    } else if (sscanf(argv[i], "--wss=%d%c", &n, &junk) == 1) {  // 22-7-11观察缓存命中率的工作集大小
      FLAGS_wss = n;
    } else if (sscanf(argv[i], "--bloom_bits=%d%c", &n, &junk) == 1) {
      FLAGS_bloom_bits = n;
    } else if (sscanf(argv[i], "--ycsb_dist=%d%c", &n, &junk) == 1) {  // 4-8 均匀分布为1，zipf为0
